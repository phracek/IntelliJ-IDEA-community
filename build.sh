...
/usr/bin/perl spectool.pl --all --get-files intellij-idea-community.spec

echo "Building SRPM..."
set -o pipefail
SRPM=$(rpmbuild -bs intellij-idea-community.spec --define "_sourcedir `pwd`" --define "_srcrpmdir $RESULT_DIR" | sed -E "s/Wrote: (.*)/\1/g")
